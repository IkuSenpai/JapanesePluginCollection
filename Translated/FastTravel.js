...
・アップデート情報
- 移動先表示スイッチ OR 移動先非表示スイッチを使ったとき移動先がおかしくなるバグ修正
...
(() => {
    ...
    function Potadra_checkVariable(variable_no, number = 0) {
        if (variable_no === 0 || $gameVariables.value(variable_no) === number) {
            return true;
        } else {
            return false;
        }
